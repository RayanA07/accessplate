...
class OnboardingTargetsStep extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return OnboardingStepLayout(
      title: copy.choose('Daily nutrition targets', 'Metas diarias de nutricion'),
      ...
      children: [
        SectionCard(
          child: Column(
            ...
            children: [
              OnboardingMetaLabel(copy.choose('Daily targets', 'Metas diarias')),
              const SizedBox(height: 10),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: [
                  Chip(label: Text('${daily.calories.toStringAsFixed(0)} kcal')),
                  Chip(label: Text('${daily.proteinG.toStringAsFixed(0)}g ${copy.proteinLabel.toLowerCase()}')),
                  Chip(label: Text('${daily.carbsG.toStringAsFixed(0)}g ${copy.carbsLabel.toLowerCase()}')),
                  Chip(label: Text('${daily.fatG.toStringAsFixed(0)}g ${copy.fatLabel.toLowerCase()}')),
                  Chip(label: Text('${daily.fiberG.toStringAsFixed(0)}g ${copy.choose('fiber', 'fibra')}')),
                ],
              ),
        ...
        SectionCard(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              OnboardingMetaLabel(copy.choose('Daily limits to watch', 'Limites diarios a cuidar')),
              const SizedBox(height: 10),
              _LimitLine(
